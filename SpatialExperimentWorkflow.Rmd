...
into the *SpatialExperiment* Bioconductor package.
...
We start to illustrate how to use the **SpatialExperiment** class by using a *seqFISH* dataset that we stored
into the SingleCellMultiModal (SCMM) Bioconductor  ExperimentHub package.
This is partially included into the seqFISH vignettes of the SCMM package.
...
developed into the **spatzli** package (thanks Lukas Weber for converting your functions for working with VisiumExperiment).
...
To annotate the spots we use the *SingleR* package with built-in reference.

Because VisiumExperiment inherits from SingleCellExperiment, it automatically works
with other packages such as *scater* and *SingleR*.
